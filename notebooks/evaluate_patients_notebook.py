...
def resized_stack(numpy_list, dsize=None):
    ...
    resized = [cv2.resize(src, dsize, interpolation = cv2.INTER_CUBIC) for src in reshaped]

    return np.stack(resized)

def compute_inference_stats(save_dir="./saved_inference", output=False, display=False):
    ...
    print(f"calculating model inferences for... {model_inferences}\n\n")
    ...
    for model_dir in model_inferences:
        patient_ID, MR_num = "*", "*"
...
def load_config(config_path, run_makelinks=False):
    ...
    return dataloader, model, device, binarize_func, save_dir, model_name, split
...
# %%
for loaded_configs in all_loaded_configs:
