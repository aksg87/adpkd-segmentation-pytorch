...
def create_label_arrays(patient_info, all_ids):
    patient_to_label = {}
    for id_ in all_ids:
        # SSFSE, FIESTA, OTHER, low LOG TKV, mid LOG TKV,
        # high LOG TKV, NUM SEQ category
    ...
    for patient, seq, mr, tkv in patient_info:
        ...
        # LOG TKV category
        # 13.6 to 15.2 interquartile range
        log_tkv = np.log(tkv)
        if log_tkv < 13.6:
            patient_to_label[patient][3] = 1
        elif 13.6 <= log_tkv < 15.1:
    ...
    # convert sequence counts to categories
...
# %%
with open("./data/stratification/strat_split_1.json", "w") as f:
