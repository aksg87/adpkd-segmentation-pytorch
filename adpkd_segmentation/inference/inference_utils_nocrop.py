...
from FindCudaGPU import find_gpu  # Added: Dom 2022-01-26 for more device flexibility
...
def load_config(config_path, run_makelinks=False, inference_path=None):
    ...
    min_mem = 4*1024  # Added 2021-08-05 at 7:41 PM -- Dom
    gpu_prefix = "cuda:"  # Added 2021-08-05 at 7:41 PM -- Dom
    if torch.cuda.is_available():
    	device_name = find_gpu(min_mem, gpu_prefix)  # Added 2021-08-05 at 7:41 PM -- Dom
    	device = torch.device(device_name)  # Added 2021-08-05 at 7:41 PM -- Dom
    else:
    	print('Warning: No GPU found. Using CPU for computation.')  # Added 2021-08-05 at 7:41 PM -- Dom
    	device = torch.device("cpu")  # Added 2021-08-05 at 7:41 PM -- Dom
...
# %% Change the a symbol of an existing string. DR: 2022-01-26 #########
def change_symbol(input_string, original, new):    
    temp_string = ""
    for index_string in range(0, len(input_string)):
        if input_string[index_string] == original and index_string != (len(input_string) - 1):
            temp_string += new
        elif input_string[index_string] == original and index_string == (len(input_string) - 1):
            print('removing ending slash from string')
        else:
            temp_string += input_string[index_string]
    ...
    for batch_idx, output in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            for dcm_path, dcm_name, file_attrib, img, logit, pred in zip(
                ...
                orig_symbol = "/"  # DR 2022-01-26. Some composed scans are abd/pel, and this makes a new folder
                new_symbol = "-"  # DR 2022-01-26. These three lines address this issue by switching the character
                valid_sequence = change_symbol(file_attrib["MR"], orig_symbol, new_symbol)  # DR 2021-12-02
                out_dir = (
                    Path.cwd()
                    / Path(save_dir)
                    / model_name
                    / file_attrib["patient"]
                    / valid_sequence # DR 2021-12-29
                    / dcm_name
                )
...
def inference_to_nifti(inference_dir, inverse_crop_ratio=1):
