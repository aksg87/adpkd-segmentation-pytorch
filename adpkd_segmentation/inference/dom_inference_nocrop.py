...
from inference_utils_nocrop import (
    load_config,
    inference_to_disk,
    display_volumes,
    inference_to_nifti,
...
def run_inference(
    ...
    inference_to_disk(**model_args)  # This is where the model is applied

    # %%
    # Creating figures for all inferences

    # Get all model inferences
    inference_files = list(Path(saved_inference).glob("**/*")) # Then Globs all this shit
    ...
    for study_dir, save_dir in tqdm(list(zip(folders, saved_folders))):
        try:
            if not os.path.isdir(study_dir / 'ITKSNAP_DCM_NIFTI'): # DOM 2022-02-03
                ...
                print("Inference already complete. Please delete directory if you wish to overwrite.")
